...
const MAX_GENERATE_CODE_RETRY_TIMES = 10;
...
function Training(params = {}) {
  ...
  this.createSyllabus = async function (args = {}, opts = {}) {
    return serviceUtil.wrap(ctx, "createSyllabus", args, opts, async function (args, opts) {
      ...
      try {
        validateSyllabusRequest(name, level, null, ctx);
...
function validateSyllabusRequest(name, level, id, ctx) {
  validateRequireInput(name, level, id, ctx);
  validateReferralType(type, typeConfigs, { language, errorBuilder });
}

function validateRequireInput(name, level, id, ctx) {
  const payload = {
    name: name,
    level: level
  };
